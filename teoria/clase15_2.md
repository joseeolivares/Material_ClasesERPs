# UPDATE, DELETE y algunos SELECT

Vamos a usar la siguiente BD:
...
UPDATE Platos
SET descrip = 'Nuevo plato principal'
WHERE nombrep = 'Plato2';
...
```SQL
SELECT nombrep, descrip, precio
FROM Platos;

SELECT *
FROM Ingredientes
WHERE nombrei LIKE '%Ingrediente%';

SELECT *
FROM Categorias
WHERE nombrec LIKE 'A%' OR nombrec LIKE 'B%';

SELECT nombrep, precio
FROM Platos
WHERE fk_id_categoria = (SELECT id FROM Categorias WHERE nombrec = 'A');

SELECT nombrep, descrip
FROM Platos
WHERE fk_id_categoria IN (SELECT id FROM Categorias WHERE encarg = 'SI');
